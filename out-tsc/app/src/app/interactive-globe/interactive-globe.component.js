var InteractiveGlobeComponent_1;
...
let InteractiveGlobeComponent = class InteractiveGlobeComponent {
    static { InteractiveGlobeComponent_1 = this; }
    ...
    constructor(platformId) {
        ...
        this.moduleChipToggle = new EventEmitter();
        ...
        this.moduleFilterExpanded = false;
        this.maxExplorerOverlayLabels = 45;
        this.maxExplorerRelationshipArcs = 20;
        this.minSharedEntitiesForArc = 2;
        this.maxEntityKeysPerLocation = 120;
        this.maxPairChecks = 800;
        this.arcInterpolationPoints = 36;
        this.sharedArcEntityTypes = new Set([
            'indicator',
            'domain-name',
            'url',
            'ipv4-addr',
        ...
        this.borderThemeProgress = 0; // 0 = purple (intel), 1 = red (no intel)
        this.borderThemeAnimId = null;
        this.isBrowser = isPlatformBrowser(platformId);
    }
    isModuleChipSelected(key) {
        return (this.selectedModuleFilterKeys ?? []).includes((key || '').trim());
    }
    isModuleChipMuted(key) {
    ...
    resolveIsoFromName(name) {
        ...
        return InteractiveGlobeComponent_1.nameAliasToIso[lower] || InteractiveGlobeComponent_1.regionNameToIsoMap.get(lower);
    }
    ...
    performOverlayRender(data) {
        ...
        const relationshipAnchorByCountry = new Map();
        ...
        for (const item of data || []) {
            ...
            const hasRenderableIntel = (Array.isArray(item?.nodes_vertex_collection) && item.nodes_vertex_collection.length > 0) ||
            ...
            if (lonlat) {
                const canRenderOverlay = !this.explorerMode || renderedOverlayLabels < this.maxExplorerOverlayLabels;
                if (!canRenderOverlay) {
                    continue;
                }
    ...
    hasAnyIntelligenceData(data) {
        ...
        return data.some((item) => {
            ...
            const nodeEntities = reportNodes.some((node) => Array.isArray(node?.entities) && node.entities.length > 0);
            return entities.length > 0 || iocs.length > 0 || highlights.length > 0 || nodeEntities;
        });
    }
    sanitizeOverlayLineText(value) {
        return value.replace(/^Article(?:\s*[:\-])?\s*/i, '').trim();
    }
    buildOverlayLines(item) {
    ...
    extractFallbackLocation(item) {
        ...
        for (const report of reportNodes) {
            ...
            for (const entity of entities) {
                ...
                const code = this.normalizeCountryCode(String(entity?.country ?? '').trim()) || '';
    ...
    buildRelationshipEntityKeys(item) {
        const keys = new Set();
        ...
        const ingest = (candidate, source) => {
            ...
            const primary = this.sanitizeOverlayLineText(String(candidate?.name ?? candidate?.value ?? candidate?.pattern ?? '').trim()).toLowerCase();
            if (!primary) {
                return;
            }
        ...
        if (visibleCandidates.length) {
            ...
            if (keys.size >= this.minSharedEntitiesForArc) {
                return keys;
            }
        }
        const reportNodes = Array.isArray(item?.nodes_vertex_collection) ? item.nodes_vertex_collection : [];
    ...
    renderRelationshipArcs(anchors) {
        if (!this.relationGroup || anchors.length < 2) {
            return;
        }
        const collectPairs = (minShared) => {
            const pairs = [];
            let checks = 0;
        ...
        const pairs = collectPairs(this.minSharedEntitiesForArc);
        if (!pairs.length) {
            return;
        }
        ...
        for (const pair of topPairs) {
            ...
            try {
                ...
                labelEl.setAttribute('title', sharedKeys.map(key => this.formatEntityKeyForDisplay(key)).join('\n'));
    ...
    buildArcPointsAboveGlobe(from, to, sharedWeight) {
    ...
    countSharedEntityKeys(left, right) {
    ...
    collectSharedEntityKeys(left, right, limit) {
    ...
    setNoIntelligenceTheme(isNoIntelligence) {
    ...
    getBaseLineTheme() {
        const mix = (from, to) => from + (to - from) * this.borderThemeProgress;
    ...
    clearOverlays() {
        ...
        if (this.relationGroup) {
            while (this.relationGroup.children.length) {
                const obj = this.relationGroup.children.pop();
                if (!obj)
                    continue;
                if (obj.geometry && typeof obj.geometry.dispose === 'function')
                    obj.geometry.dispose();
...
], InteractiveGlobeComponent.prototype, "containerRef", void 0);
__decorate([
    HostBinding('class.stretch-globe')
], InteractiveGlobeComponent.prototype, "stretchClass", void 0);
__decorate([
    HostBinding('class.landing-mode')
], InteractiveGlobeComponent.prototype, "landingModeClass", null);
__decorate([
    HostBinding('class.explorer-mode')
], InteractiveGlobeComponent.prototype, "explorerModeClass", null);
__decorate([
    Output()
], InteractiveGlobeComponent.prototype, "selectedCountryChange", void 0);
__decorate([
    Output()
], InteractiveGlobeComponent.prototype, "entityGroupsChange", void 0);
__decorate([
    Input()
], InteractiveGlobeComponent.prototype, "selectedCountry", void 0);
__decorate([
    Input()
], InteractiveGlobeComponent.prototype, "selectedCountryCode", void 0);
__decorate([
    Input()
], InteractiveGlobeComponent.prototype, "selectedCountryGroups", void 0);
__decorate([
    Input()
], InteractiveGlobeComponent.prototype, "selectedCountryLoading", void 0);
__decorate([
    Input()
], InteractiveGlobeComponent.prototype, "explorerMode", void 0);
__decorate([
    Input()
], InteractiveGlobeComponent.prototype, "autoFocusOnSelection", void 0);
__decorate([
    Input()
], InteractiveGlobeComponent.prototype, "enableDateFilter", void 0);
__decorate([
    Input()
], InteractiveGlobeComponent.prototype, "dateRangeStart", void 0);
__decorate([
    Input()
], InteractiveGlobeComponent.prototype, "dateRangeEnd", void 0);
__decorate([
    Input()
], InteractiveGlobeComponent.prototype, "dateRangeMin", void 0);
__decorate([
    Input()
], InteractiveGlobeComponent.prototype, "dateRangeMax", void 0);
__decorate([
    Input()
], InteractiveGlobeComponent.prototype, "moduleBreakdownCounts", void 0);
__decorate([
    Input()
], InteractiveGlobeComponent.prototype, "selectedModuleFilterKeys", void 0);
__decorate([
    Output()
], InteractiveGlobeComponent.prototype, "dateRangeChange", void 0);
__decorate([
    Output()
], InteractiveGlobeComponent.prototype, "moduleChipToggle", void 0);
__decorate([
    Input()
], InteractiveGlobeComponent.prototype, "selectedCountryIocs", null);
__decorate([
    Input()
], InteractiveGlobeComponent.prototype, "stretch", null);
InteractiveGlobeComponent = InteractiveGlobeComponent_1 = __decorate([
    Component({
...
export { InteractiveGlobeComponent };
